...
for i = [1,4,9,12,15,16,19,24,27,30,31] %hardcoded for number of files
    num = num2str(i);
    name = append(run,num);
...
title('Collaborative Design Rounds')
axis([0, 1.1*max(indexledger), 0, 1.1*max(iterledger)])
saveas(gcf,'percentchangecollab.fig')
saveas(gcf,'percentchangecollab.png')
...
saveas(gcf,'numchangedcollab.fig')
saveas(gcf,'numchangedcollab.png')
